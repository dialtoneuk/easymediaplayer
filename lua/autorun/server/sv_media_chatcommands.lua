...
MediaPlayer.RegisteredCommands = {
	...
	VoteSkip = {
		...
		OnExecute = function(ply, cmd)
			ply:ConCommand("MediaPlayer_start_vote" .. " VoteSkip")
		end,
		Aliases = {
			"vs"
		}
	...
	VoteBlacklist = {
		...
		OnExecute = function(ply, cmd)
			...
			--sv_media_voting.lua
			ply:ConCommand("MediaPlayer_start_vote" .. " VoteBlacklist")
		end,
		Aliases = {
			"voteban",
			"vb"
	...
	Search = {
		OnExecute = function(ply, cmd)
			ply:ConCommand("MediaPlayer_search_panel")
		end
	},
	Settings = {
		OnExecute = function(ply, cmd)
			ply:ConCommand("MediaPlayer_settings")
		end,
		Aliases = {
			"s"
		}
	...
	Like = {
		OnExecute = function(ply, cmd)
			...
			ply:ConCommand("MediaPlayer_like_video")
		end
	},
	Dislike = {
		OnExecute = function(ply, cmd)
			...
			ply:ConCommand("MediaPlayer_dislike_video")
		end
	},
	Mute = {
		OnExecute = function(ply, cmd)
			...
			if (MediaPlayer.GetSeting("MediaPlayer_mute_video").Value) then
				MediaPlayer.ChangeSetting("MediaPlayer_mute_video", false );
				ply:SendMessage("Video unmuted")
			else
				MediaPlayer.ChangeSetting("MediaPlayer_mute_video", true );
				ply:SendMessage("Video muted")
			end

			ply:RunConsoleCommand("MediaPlayer_create_cl");
		end,
		Aliases = {
			"m",
			"u",
	...
	Blacklist = {
		Admin = true,
		OnExecute = function(ply, cmd)
			ply:ConCommand("MediaPlayer_blacklist_video")
		end
	},
	Skip = {
		Admin = true,
		OnExecute = function(ply, cmd)
			ply:ConCommand("MediaPlayer_skip_video")
		end
	},
	Admin = {
		Admin = true,
		OnExecute = function(ply, cmd)
			ply:ConCommand("MediaPlayer_admin_panel")
		end
	}
}
...
function MediaPlayer.ParseCommand(ply, str)
	if (str.sub(str, 1,1) != MediaPlayer.GetSetting("MediaPlayer_command_prefix").Value ) then return false end
